...
class BusinessAndConfluenceService
{
    ...
    /**
     * BusinessAndConfluenceService constructor.
     */
    ...
    public function connectConfluence()
    {
        ...
        $this->_confluence->instance->callbackNewData      = [$this, 'callbackConfluenceNewData'];
        $this->_confluence->instance->callbackConnectClose = [$this, 'callbackConfluenceConnectClose'];
        $this->_confluence->confluence                     = [];
        $this->_confluence->connect();
        $result = $this->_confluence->send(['token'    => '',
                                            'msg_type' => MsgTypeConst::MSG_TYPE_ADD_BUSINESS_TO_CONFLUENCE
        ]);
        if ($result === false) {
    ...
    public function callbackConfluenceNewData($connect, $data)
    {
        switch ($data['msg_type']) {
            ...
            case MsgTypeConst::MSG_TYPE_RESET_TRANSFER_LIST:
                ...
                break;
            default:
                Log::write('Business: Confluence message type is not supported, data=' . json_encode($data) . ', client address: ' . json_encode($connect->getClientAddress()),
                    'ERROR');
                $this->_closeConfluence();
    ...
    private function _addConfluenceResponse($connect, $data)
    {
        ...
        $this->_confluence->confluence['waiter_confluence_ping_timer_id'] = Timer::add(function () {
            if ((++$this->_confluence->confluence['confluence_no_ping_limit']) >= TRIDENT_SYS_PING_NO_RESPONSE_LIMIT) {
                //断开链接
                $this->_closeConfluence();
            }
    ...
    {
        Timer::add([$this, 'connectConfluence'], [], 1, false);
    }

...
class Confluence extends MeepoPS
{
     ...
     */
    public function __construct($protocol, $host, $port, array $contextOptionList = [])
    {
        $this->callbackStartInstance = [$this, 'callbackConfluenceStartInstance'];
        $this->callbackConnect       = [$this, 'callbackConfluenceConnect'];
        $this->callbackNewData       = [$this, 'callbackConfluenceNewData'];
        $this->callbackConnectClose  = [$this, 'callbackConfluenceConnectClose'];
    ...
    public function callbackConfluenceStartInstance()
    {
        ...
        }, [], TRIDENT_SYS_CONFLUENCE_BROADCAST_INTERVAL);
    }
    ...
    public function callbackConfluenceConnect($connect)
    {
        $connect->confluence['waiter_verify_timer_id'] = Timer::add(function ($connect) {
            Log::write('Confluence: Wait for token authentication timeout. client address: ' . json_encode($connect->getClientAddress()),
                'ERROR');
            $this->_close($connect);
        }, [$connect], TRIDENT_SYS_WAIT_VERIFY_TIMEOUT, false);
    }
        ...
        switch ($data['msg_type']) {
            ...
            case MsgTypeConst::MSG_TYPE_ADD_TRANSFER_TO_CONFLUENCE:
                ...
                if (!isset($data['token']) || Tool::verifyAuth($data['token']) !== true) {
                    Log::write('Confluence: New link token validation failed, client address: ' . json_encode($connect->getClientAddress()),
                        'ERROR');
                    $this->_close($connect);
                ...
                if ($result) {
                    ...
                    Log::write('Confluence: _addTransfer return result: ' . $result . ', client address: ' . json_encode($connect->getClientAddress()),
                        'ERROR');
                }
                break;
            // 新的Business加入
            case MsgTypeConst::MSG_TYPE_ADD_BUSINESS_TO_CONFLUENCE:
                ...
                if ($result) {
                    ...
                    Log::write('Confluence: _addBusiness return result: ' . $result . ', client address: ' . json_encode($connect->getClientAddress()),
                        'ERROR');
                }
                break;
            // PONG
            case MsgTypeConst::MSG_TYPE_PONG:
                ...
                break;
            default:
                Log::write('Confluence: New link message type is not supported, client address: ' . json_encode($connect->getClientAddress()) . ', data=' . json_encode($data),
                    'ERROR');
                $this->_close($connect);
    ...
    private function _addTransfer($connect, $data)
    {
        ...
        $connect->confluence['ping_timer_id'] = Timer::add(function ($connect) {
            $connect->send(['msg_type' => MsgTypeConst::MSG_TYPE_PING, 'msg_content' => 'PING']);
        }, [$connect], TRIDENT_SYS_PING_INTERVAL);
        // 检测PING回复情况
        $connect->confluence['check_ping_timer_id'] = Timer::add([$this, 'checkPingLimit'], [$connect], TRIDENT_SYS_PING_INTERVAL);
        $this->broadcastToBusiness();

        // 告知对方, 已经收到消息, 并且已经添加成功了
        return $connect->send(['msg_type' => MsgTypeConst::MSG_TYPE_ADD_TRANSFER_TO_CONFLUENCE, 'msg_content' => 'OK']);
    }
    ...
    private function _addBusiness($connect, $data)
    {
        ...
        // 检测PING回复情况
        $connect->confluence['check_ping_timer_id'] = Timer::add([$this, 'checkPingLimit'], [$connect],
            TRIDENT_SYS_PING_INTERVAL);
        $this->broadcastToBusiness($connect);

        // 告知对方, 已经收到消息, 并且已经添加成功了
        return $connect->send(['msg_type' => MsgTypeConst::MSG_TYPE_ADD_BUSINESS_TO_CONFLUENCE, 'msg_content' => 'OK']);
    }
    ...
    public function checkPingLimit($connect)
    {
        $connect->confluence['ping_no_response_count']++;
        // 超出无响应次数限制时断开连接
        if (($connect->confluence['ping_no_response_count'] - 1) >= TRIDENT_SYS_PING_NO_RESPONSE_LIMIT) {
            $conn = '';
            ...
            Log::write('Confluence: PING no response beyond the limit, has been disconnected, client address: ' . json_encode($connect->getClientAddress()),
                'ERROR');
            $this->_close($connect);
        }
    }

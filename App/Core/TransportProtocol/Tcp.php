...
class Tcp extends TransportProtocolInterface
{
    ...
    public function __construct($socket, $clientAddress, $applicationProtocolClassname = '')
    {
        ...
        //监听此链接
        MeepoPS::$globalEvent->add([$this, 'read'], [], $this->_connect, EventInterface::EVENT_TYPE_READ);
    }
    ...
    private function _readByApplicationProtocol()
    {
        ...
        while (!empty($this->_readData) && $this->_isPauseRead === false) {
            ...
            if ($this->_currentPackageSize > MEEPO_PS_TCP_CONNECT_READ_MAX_PACKET_SIZE) {
                ...
                Log::write('data packet size exceeds the maximum limit. size=' . $this->_currentPackageSize . '. limit=' . MEEPO_PS_TCP_CONNECT_READ_MAX_PACKET_SIZE,
                    'WARNING');
                //强制销毁链接, 该链接尚未发送的数据也不发了.
                $this->destroy();
    ...
    public function send($data, $isEncode = true)
    {
        ...
        if ($length > 0 && $length === strlen($data)) {
            ...
            MeepoPS::$globalEvent->add([$this, 'sendEvent'], [], $this->_connect, EventInterface::EVENT_TYPE_WRITE);
            //检测队列是否为空
            $this->_sendBufferIsFull();
        ...
        if ($length === strlen($this->_sendBuffer)) {
            ...
            if (!empty($this->instance->callbackSendBufferEmpty)) {
                try {
                    ...
                    self::$statistics['exception_count']++;
                    Log::write('MeepoPS: execution callback function callbackSendBufferEmpty-' . json_encode($this->instance->callbackSendBufferEmpty) . ' throw exception' . json_encode($e),
                        'ERROR');
                }
            }
            ...
            if (!empty($this->instance->callbackError)) {
                try {
                    call_user_func_array($this->instance->callbackError, [
                        $this,
                        MEEPO_PS_ERROR_CODE_SEND_SOCKET_INVALID,
    ...
    public function destroy()
    {
        ...
        if (!empty($this->instance->callbackConnectClose)) {
            try {
                ...
                self::$statistics['exception_count']++;
                Log::write('MeepoPS: execution callback function callbackConnectClose-' . json_encode($this->instance->callbackConnectClose) . ' throw exception' . json_encode($e),
                    'ERROR');
            }
        }
        unset($this);

...
class Event implements EventInterface
{
    ...
    public function add($callback, array $args, $resource, $type)
    {
        ...
        switch ($type) {
            ...
            case self::EVENT_TYPE_SIGNAL:
                $eventType = $type === self::EVENT_TYPE_READ ? (EV_READ | EV_PERSIST) : ($type === self::EVENT_TYPE_WRITE ? (EV_WRITE | EV_PERSIST) : (EV_SIGNAL | EV_PERSIST));
                $uniqueId  = (int)($resource);
                // 2.准备想要在event_add中添加事件
                if (event_set($event, $resource, $eventType, $callback, $args) // 3.关联事件到事件base
                    && event_base_set($event, $this->_eventBase) // 4.向指定的设置中添加一个执行事件
                    && event_add($event)
                ) {
            ...
            case self::EVENT_TYPE_TIMER_ONCE:
                ...
                if (event_set($event, 0, EV_TIMEOUT, [$this, 'timerCallback'], $timerId) && event_base_set($event,
                        $this->_eventBase) && event_add($event, $intervalMicrosecond)
                ) {
                    $this->_eventList[$type][$timerId] = [$callback, (array)$args, $event, $type, $intervalMicrosecond];
                ...
                return false;
            default :
                Log::write('Event: add failed. ' . $type . ' is unrecognized type', 'WARNING');
    ...
    public function timerCallback($_null, $_null, $timerId)
    {
        if (isset($this->_eventList[self::EVENT_TYPE_TIMER][$timerId])) {
            ...
            Log::write('Event: timer event handle failed. timer id is ' . $timerId, 'WARNING');
        ...
        try {
            ...
            Log::write('YP_Socket: execution callback function timer callback-' . $timer[0] . ' throw exception' . json_encode($e),
                'ERROR');
        }
    }
}
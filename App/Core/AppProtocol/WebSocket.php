...
class WebSocket implements AppProtocolInterface
{
    ...
    private static function _parseHttpHeader($data, TransportProtocolInterface $connect)
    {
        ...
        $_SERVER['SERVER_SOFTWARE'] = 'YP_Socket/' . YP_VERSION . '( ' . PHP_OS . ' ) PHP/' . PHP_VERSION;
    ...
    private static function _unWrap(&$allFrameLength, $data, $connect, $dataLength)
    {
        ...
        if ($payLoadLen === 126) {
            ...
            Log::write('WebSocket protocol received data errors, because the payloadlen is not correct. payloadlen=' . $payLoadLen,
                'WARNING');
            self::_disConnect($connect);
            ...
            case 0x9:
                ...
                if ($connect->instance->callbackWSPing) {
                    try {
                        ...
                        Log::write('YP_Socket: execution callback function callbackWSPing-' . json_encode($connect->instance->callbackWSPing) . ' throw exception' . json_encode($e),
                            'ERROR');
                    }
                } else {
            ...
            case 0xa:
                ...
                if ($connect->instance->callbackWSPong) {
                    try {
                        ...
                        Log::write('YP_Socket: execution callback function callbackWSPong-' . json_encode($connect->instance->callbackWSPong) . ' throw exception' . json_encode($e),
                            'ERROR');
                    }
                }
                // 如果数据中消息的长度为假,则从接收数据缓冲区中删除规定的消息头部分.
    ...
    private static function _disConnect($connect)
    {
        if ($connect->instance->callbackWSDisconnect) {
            try {
                ...
                Log::write('YP_Socket: execution callback function callbackWSDisconnect-' . json_encode($connect->instance->callbackWSDisconnect) . ' throw exception' . json_encode($e),
                    'ERROR');
            }
        }
        $connect->close();

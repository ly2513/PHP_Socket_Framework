...
class Http implements AppProtocolInterface
{
    ...
    public static function setCookie(
        ...
        return self::setHeader('Set-Cookie: ' . $name . '=' . urlencode($value) . (empty($domain) ? '' : '; Domain=' . $domain) . (empty($maxage) ? '' : '; Max-Age=' . $maxage) . (empty($path) ? '' : '; Path=' . $path) . (empty($secure) ? '' : '; Secure') . (empty($httpOnly) ? '' : '; HttpOnly'),
            false);
    }
        ...
        foreach ($postDataMapList as $postDataMap) {
            ...
            if (preg_match('/; name="(.*)"/Ui', $postDataMap[0], $name)) {
                ...
                if (!preg_match('/; filename="(.*)"/', $postDataMap[0], $filename)) {
                    ...
                    $_FILES[$name[1]]['type'] = preg_match('/Content-Type: ?(.*);?/s', $postDataMap[0],
                        $contentType) ? $contentType[1] : '';
                    if (MEEPO_PS_HTTP_UPLOAD_FILE_GENERATE_TEMP_FILE && ($filename = tempnam(sys_get_temp_dir(),
                            'uf_')) && $filename !== false && file_put_contents($filename, $postDataMap[1]) !== false
                    ) {
                ...
                //日志
                Log::write('An illegal HTTP POST request, because the name does not exist. HTTP body: ' . $postDataMap[0],
                    'WARNING');
            }
        }
        //填充$_POST

...
class TcpClient extends Tcp
{
    ...
    {
        //传入的协议是应用层协议还是传输层协议
        $protocol = '\App\Core\ApplicationProtocol\\' . ucfirst($protocol);
        if ($protocol) {
            if (class_exists($protocol)) {
                $this->_protocol                     = '\App\Core\ApplicationProtocol\\' . $protocol;
    ...
    public function connect()
    {
        ...
        //监听此链接
        YP_Socket::$globalEvent->add([$this, 'checkConnection'], [], $this->_connect, EventInterface::EVENT_TYPE_WRITE);
    }
    ...
    public function checkConnection($tcpConnect)
    {
        ...
        YP_Socket::$globalEvent->delOne($tcpConnect, EventInterface::EVENT_TYPE_WRITE);
        ...
        YP_Socket::$globalEvent->add([$this, 'read'], [], $tcpConnect, EventInterface::EVENT_TYPE_READ);
        if ($this->_sendBuffer) {
            YP_Socket::$globalEvent->add([$this, 'sendEvent'], [], $tcpConnect, EventInterface::EVENT_TYPE_WRITE);
        }

...
class Http extends YP_Socket
{
    // 默认页文件名
    private $_defaultIndexList = [];
     ...
     */
    public function __construct($host, $port, $contextOptionList = [])
    {
        if (!$host || !$port) {
            return;
        }
        ...
        // 域名和目录
        $domainDocumentList = explode('|', HTTP_DOMAIN_DOCUMENT_LIST);
        ...
        // 默认页
        $this->_defaultIndexList = explode(',', HTTP_DEFAULT_PAGE);
    }
    ...
    public function callbackNewData($connect, $data)
    {
        if (!empty($this->_userCallbackNewData)) {
            try {
                call_user_func_array($this->_userCallbackNewData, [$connect, $data]);
            } catch (\Exception $e) {
                Tcp::$statistics['exception_count']++;
                Log::write('MeepoPS: execution callback function callbackNewData-' . json_encode($this->_userCallbackNewData) . ' throw exception' . json_encode($e),
                    'ERROR');
            }
        }
